...
class CategoryDataset(Dataset):
    """Dataset for polyvore with 5 categories(upper, bottom, shoe, bag, accessory),
        ...
        """
    def __init__(self,
                 root_dir="C:/Users/anastasia/Desktop/data/images/",
                 data_file='train_no_dup_with_category_3more_name.json',
                 data_dir="C:/Users/anastasia/Desktop/data",
                 transform=None,
                 use_mean_img=True,
                 neg_samples=True):
    ...
    def __getitem__(self, index):
        ...
        print("Value of self.data[index]:", type(self.data[index]), set_id, parts)
        ...
        for part in ['upper', 'bottom', 'shoe', 'bag', 'accessory']:
            if part in to_change: # random choose a image from dataset with same category
                ...
                img_path = self.root_dir+str(choice[0])+'/'+ str(choice[1][part]['index'])+'.jpg' #os.path.join(self.root_dir, str(choice[0]), str(choice[1][part]['index'])+'.jpg')
                ...
                img_path = self.root_dir+str(set_id)+'/'+ str(parts[part]['index'])+'.jpg' #os.path.join(self.root_dir, str(set_id), str(parts[part]['index'])+'.jpg')
                ...
                img_path = self.data_dir+'/'+ part+'.png' #os.path.join(self.data_dir, part+'.png')
        ...
        print(f'Input images dimensions: {input_images.shape}')
        print(f'Names dimensions: {[name.shape for name in names]}')
        is_compat = (len(to_change) == 0)

        offsets = list(itertools.accumulate([0] + [len(n) for n in names[:-1]]))
        offsets = torch.LongTensor(offsets)
        print(f'Offsets dimensions: {offsets.shape}')
        print(f'Labels dimensions: {len(labels)}')
...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        set_id, parts = self.data[index]
        print("Value of self.data[index]:", set_id, parts)
        choice = random.sample(list(parts.keys()), k=2)
        print(choice)
        anchor_part, pos_part = choice[0], choice[1]
        print("BE:anchor_part, pos_part:", anchor_part, pos_part) #كل مرة نتيجة بشكل عشوائي يعطي
        ...
        print("After:anchor_part, pos_part:", anchor_part, pos_part)
...
if __name__ == "__main__":
    ...
    dataset_instance = CategoryDataset(root_dir="C:/Users/anastasia/Desktop/data/images/",
                                       data_file='train_no_dup_with_category_3more_name.json',
                                       data_dir="C:/Users/anastasia/Desktop/data",
                                       transform=transform,
                                       use_mean_img=True,
    ...
    with open("C:/Users/anastasia/Desktop/data/train_no_dup_with_category_3more_name.json") as f:
        df = json.load(f)

    for i,(_,v) in enumerate(df.items()):
        if len(v.keys())==5:
            print(i)
            break
    ...
    data = TripletDataset(root_dir="C:/Users/anastasia/Desktop/data/images/",
                                       data_file='train_no_dup_with_category_3more_name.json',
                                       data_dir="C:/Users/anastasia/Desktop/data",
                                       transform=transform,
                                       is_train=True
