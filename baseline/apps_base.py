...
from utilsmcn import prepare_dataloaders
from PIL import Image

data_root = "C:/Users/anastasia/Desktop/data"
img_root = "C:/Users/anastasia/Desktop/data/images"
...
model.load_state_dict(torch.load("./model_mcn.pth", map_location="cpu"))
model.eval()
...
def item_diagnosis(relation, select):
    ...
    mats = vec2mat(relation, select)
    for m in mats:
        mask = torch.eye(*m.shape, dtype=torch.bool)  # تحديد النوع ليكون بولياني
        m = torch.where(mask, torch.tensor(0), m)    # استخدام torch.where لتطبيق القناع بشكل بولياني
    result = torch.cat(mats).sum(dim=0)
    order = [i for i, j in sorted(enumerate(result), key=lambda x: x[1], reverse=True)]
    return result, order
...
body = dbc.Row([
        ...
        html.Div([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dcc.Upload(id="upload-top", children=['Drop here or ', html.A('Upload')],
                               style={"textAlign": "center", "border": "1px dashed black", "line-height": "34px",
        ...
        html.Div([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dcc.Upload(id="upload-bottom", children=['Drop here or ', html.A('Upload')],
                               style={"textAlign": "center", "border": "1px dashed black", "line-height": "34px",
                                      "height": "34px", "border-radius": "5px"})
                ])
            ])
        ...
        html.Div([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dcc.Upload(id="upload-shoe", children=['Drop here or ', html.A('Upload')],
                               style={"textAlign": "center", "border": "1px dashed black", "line-height": "34px",
                                      "height": "34px", "border-radius": "5px"})
                ])
            ])
        ...
        html.Div([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dcc.Upload(id="upload-bag", children=['Drop here or ', html.A('Upload')],
                               style={"textAlign": "center", "border": "1px dashed black", "line-height": "34px",
                                      "height": "34px", "border-radius": "5px"})
                ])
            ])
        ...
        html.Div([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dcc.Upload(id="upload-accessory", children=['Drop here or ', html.A('Upload')],
                               style={"textAlign": "center", "border": "1px dashed black", "line-height": "34px",
                                      "height": "34px", "border-radius": "5px"})
                ])
            ])
    ...
    dbc.Col(html.Div([
        ...
        dbc.Card(dbc.CardBody([
            dcc.Loading(id="loading-output", children=[html.Div(html.H5("Output afte submisson"), id="output-state")],
                        type="default")
        ])),
    ]), width=12, lg=8
    ),
...
README = open('C:/Users/anastasia/Desktop/fashion_compatibility_mcn-master/README.md').read()

copyright = html.Center(["© 2024 Copyright by ", html.A("... ", href=""), "."])
...
def update_output(n_clicks, try_most, top, bottom, shoe, bag, accessory, top_style, bottom_style, shoe_style, bag_style,
                  accessory_style):
    ...
    if n_clicks:
        ...
        if score > 0.9:
            original_score = html.H5(children="Original Score: {:.4f}. This outfit is compatible.".format(score),
                                     style={"color": "green"})
        ...
        end_time = time.time()
        out.append(html.H5(children="Execute time for diagnosis: {:.4f}s".format(end_time - start_time),
                           style={"color": "black"}))
        ...
        return [html.H5("Output after submisson"), None, top_style, bottom_style, shoe_style, bag_style,
                accessory_style]
