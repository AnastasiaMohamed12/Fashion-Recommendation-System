...
class CompatModel(nn.Module):
    def __init__(
        ...
        if self.mlp_layers > 0:
            ...
            for _ in range(self.mlp_layers-1):
                ...
                nn.init.xavier_uniform_(linear.weight) # يُعدل أوزان الطبقات الخطية والتضمين
    ...
    def _compute_score(self, images, activate=True):
        ...
        # print(
        #     f'Original dimensions: batch_size={batch_size}, item_num={item_num}, img_size={img_size}')  # طباعة الأبعاد الأصلية

        images = torch.reshape(images, (-1, 3, img_size, img_size))
        # print(f'Reshaped images dimensions: {images.shape}')
        if self.need_rep:
            features, *rep = self.cnn(images)
            rep_l1, rep_l2, rep_l3, rep_l4, rep = rep
        else:
            features = self.cnn(images)
        # print(f'Features dimensions: {features.shape}')

        relations = []# حساب العلاقات بين الاقنعة والميزات
        features = features.reshape(batch_size, item_num, -1)  # (32, 5, 1000)
        # print(f'Reshaped features dimensions: {features.shape}')
...
model = CompatModel(embed_size=1000, need_rep=False, vocabulary=10000, vse_off=False, pe_off=False, mlp_layers=2, conv_feats="1234")
print(model)

print(model.num_rela)
